 ...
 * Date: ${.now}
 */
public class ${classNamePrefix}Activity extends com.github.pingia.ui.framework.base.BaseSingleFragmentActivity
{
    public static void startMe(Activity fromActivity) {
        Intent intent = new Intent(fromActivity, ${classNamePrefix}Activity.class);
        <#if (pageApi.singleFragmentClassQualifiedClassName)??> <#-- 如果配置了需要弹出请求对话框 -->
         intent.putExtra(IntentConstant.KEY_FRAGMENT_CLASS, ${pageApi.singleFragmentClassQualifiedClassName}.class);
         </#if>
